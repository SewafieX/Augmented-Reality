...
while True:
    ...
    if ret == True and processing:
        # pts2 is used for defining the perspective transform (POS of 3 of 54 corners: 1(0), 8(9), 54(53), 46(45))
        pts2 = np.float32([corners[0, 0], corners[8, 0], corners[len(corners)-1, 0], corners[len(corners)-9, 0]])
        # Function will find all the corners within the 9x6 box
        print("Length of corners: ", len(corners))
        # and return a 3D array, one of which is empty. Thus the 0
        print("Points are: ", np.float32([corners[0, 0], corners[8, 0], corners[len(corners)-1, 0], corners[len(corners)-9, 0]]))
